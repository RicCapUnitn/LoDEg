    ...
    def collectDataFromDb(self, systemInfo:dict, user: str = None):
        if (user is None):
            ...
            if (cursor is not None):
                ...
                systemInfo['last_update'] = utils.getTimeFromObjectId(cursor['_id'])
        else:
            # User level info
            cursor = self._collection.find_one({'cache_user_id': user}, {'_id': 0})
            if (cursor is not None):
                systemInfo['users'][cursor['cache_user_id']] = cursor['userInfo']
        return systemInfo
...
class CacheSQLite(Cache):
    ...
    def saveDataToDb(self, systemInfo:dict, user: str = None):
        ...
        if (user is None):
            ...
            user = LodegUser.objects.get(lodeg_user_id = user)
        DjangoCache.objects.filter(user = user).delete() # Might not be necessary
