...
find tools/tmp/migration_backups -type f -name "*.py" -print0 | xargs -0 sed -i "s/from \([\.]\+\)\(.*\) import\(.*migrate\)/from \1lodegML\.\2 import\3/g"
...
find tools/tmp/migration_backups -maxdepth 2 -type f -name "*.py" -exec md5sum {} \; |cut -c-32 |sort > tools/tmp/libmd5.txt
